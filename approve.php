...
$pageparams = new att_approve_page_params();
...
$mform = new mod_attendance_student_tag_form(null,
        array('course' => $course, 'cm' => $cm, 'modcontext' => $PAGE->context, 'attendance' => $att, 'radio' => 1));

if ($fromform = $mform->get_data()) {
    foreach($fromform as $key => $value) {
        if ($key == 'id' || $key == 'submitbutton') continue;
    ...
    redirect($att->url_approve());
}

$PAGE->set_url($att->url_approve(array('id' => $id)));
$PAGE->set_title($course->shortname. ": ".$att->name);
$PAGE->set_heading($course->fullname);
$PAGE->set_cacheable(true);
$PAGE->set_button($OUTPUT->update_module_button($cm->id, 'attendance'));
// Add get_string('train', 'attendance');
$PAGE->navbar->add('Approve');

$output = $PAGE->get_renderer('mod_attendance');
$tabs = new attendance_tabs($att, attendance_tabs::TAB_APPROVE);
