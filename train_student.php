...
require_once(dirname(__FILE__).'/student_tag_form.php');
...
$mform = new mod_attendance_student_tag_form(null,
        array('course' => $course, 'cm' => $cm, 'modcontext' => $PAGE->context, 'session' => $attforsession, 'attendance' => $att, 'radio' => 0));
...
if ($mform->is_cancelled()) {
    ...
    foreach($fromform as $key => $value) {
        if ($key == 'sessid' || $key == 'submitbutton') continue;
        
        $dataobject = $DB->get_record('attendance_images', array('faceimg' => $key));
        $dataobject->studentid = $USER->id;
        $dataobject->approved = 1;
        $success = $DB->update_record('attendance_images', $dataobject);
    }
    $url = new moodle_url('/mod/attendance/view.php', array('id' => $cm->id));
    redirect($url);
...
$url = new moodle_url('/mod/attendance/train_student.php', array());
$PAGE->set_url($url);
$PAGE->set_title("Training data");
// use get_string
$PAGE->set_heading("Show us who you are:");
